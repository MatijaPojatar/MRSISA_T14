    ...
    export default{
        ...
        methods:{
             async loadLekovi(){
                 const lekovi = []
                if (!this.apotekaId){
                    await Vue.axios.get(`/lekovi`).then(response => {
             ...
             async loadAlergije(){
                 await Vue.axios.get(`/pacijent/alergije/${this.pacijentId}`).then(response=>{
                     const alergije=[]
                     response.data.forEach(element => {
                         alergije.push({
                             id: element.id,
             ...
             async addLek(lek){
                 if(lek.alergija!=0){
                    ...
                    await Vue.axios.get(`/apoteke/stanje/${lek.id}`,{params:{apotekaId:this.apotekaId,kolicina:1}}).then(response=>{
             ...
             IzmeniLek(){
                 ...
                 Vue.axios.put(`/apoteke/preuzmiLek/${this.selektovan}`, this.apotekaId, {headers: {"Content-Type": "text/plain"}}).then(response => {
             ...
             async zamenskiDobavi(){
                 await Vue.axios.get(`/lekovi/zamenski/${this.selektovan}/${this.apotekaId}/${this.pacijentId}`).then(response=>{
                     this.obavestenjeDialog=false;
